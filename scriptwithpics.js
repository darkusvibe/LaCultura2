...
document.querySelectorAll('.buy-now').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    const price = Math.floor(Math.random() * 3000 + 500); // temporary price
...
function updateCart() {
  cartItems.innerHTML = cart.map(item =>
    `<div class="cart-item"><span>${item.name}</span><span>Ksh ${item.price}</span></div>`
  ).join('');
  cartCount.textContent = cart.length;
  cartTotal.textContent = cart.reduce((sum, i) => sum + i.price, 0);
}

// SELL FORM: Add your own product preview
  ...
  reader.onload = () => {
    const productHTML = `
      <div class="card attire-card">
        ...
        <h3>${name}</h3>
        <p>${desc}</p>
        <button class="buy-now">Buy Now (Ksh ${price})</button>
      </div>
    `;
    document.getElementById('my-products').insertAdjacentHTML('beforeend', productHTML);
  };
